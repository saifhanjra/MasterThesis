# -*- coding: utf-8 -*-
"""
Created on Thu Feb 18 12:53:38 2016
...
input_file=open('params_cnn_mb_1_20.pkl','rb')
...
#        self.accurracy=test_network(self.s)
    ...
    def create_adversial_inpt(self,test_data,mini_batchsize,epislon):
        ...
        f_test_inpt=theano.function([n],outputs=test_x[n])      ### getting input function
        self.adv_inpt_accum=[]
        ...
        for i in xrange(len(self.index_correctly_classified_list)):
            ...
            grad_correctly_classified=f_grad_x(self.test_x_correctly_classified)
            magnitude_grad_correctly_classified=np.linalg.norm(grad_correctly_classified)
            ...
            if magnitude_grad_correctly_classified>0.1:
                
                self.inpt_high_normalized_grad_all_list.append(magnitude_grad_correctly_classified)
                self.inpt_high_normalized_grad_all_index.append(self.test_x_correctly_classified)
                
                given_test_inpt=f_test_inpt(self.test_x_correctly_classified)

                self.hypothetic_adv_inpt=given_test_inpt+epislon*(grad_correctly_classified/magnitude_grad_correctly_classified)
                check_adversarity_hypothetic_inpt=self.check_adv_inpt(mini_batchsize)
                if check_adversarity_hypothetic_inpt==0:
                    
                    
                    a=self.harnesssing_adv_inpt()
                    self.adv_inpt_accum.append(a)
                    
                    self.inpt_high_normalized_grad_adv_list.append(magnitude_grad_correctly_classified)
                ...
                if check_adversarity_hypothetic_inpt==1:
                    self.inpt_high_normalized_grad_hypothetic_list.append(magnitude_grad_correctly_classified)
