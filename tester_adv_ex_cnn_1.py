# -*- coding: utf-8 -*-
"""
Created on Thu Jan 28 15:44:00 2016
...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 20 15:23:16 2016
    ...
    def create_adversial_inpt(self,test_data,mini_batchsize,epislon):
        if self.accurracy==1: ###if given test input is correctly classified
            ...
            if a==1:
                ...
                plt.xlabel(r'$\displaystyle \epsilon=0.015$',fontsize=22, color='black')
                plt.imshow(grad_inpt_sign_epislon.reshape(28,28), plt.cm.gray)
...
#            
        ...
        if self.adv_check==1:
            print 'higher the value of epsilon'
            plt.subplot(133)
            plt.title(r'$\displaystyle X_{adv}^{(i)}=X^{(i)}+\epsilon\left(sgn(\frac{\partial}{\partial X_i} cost)\right)$',fontsize=22, color='black')
            plt.imshow(self.adv_inpt.reshape(28,28),plt.cm.gray)
