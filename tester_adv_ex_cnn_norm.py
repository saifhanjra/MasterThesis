# -*- coding: utf-8 -*-
"""
Created on Mon Feb 01 18:01:36 2016
...
class Network():
    ...
    def create_adversial_inpt(self,test_data,mini_batchsize,epislon):
        if self.accurracy==1: ###if given test input is correctly classified
            print 'Given  test input is correctly classified lets make it adversarial'
            ...
            if a==1:
                ...
                f_grad_x=theano.function([m],outputs=self.grad_x,givens=
                 ...
                 ########## I want to check with how much confidence my image is going to correctly
                #### classified, it seems to be very easy
                k=T.lscalar()
                ...
                '''x_adv= x + epislon*(grad(cost,x)/norm(grad(cost,x))''' 
                ...
                norm=np.linalg.norm(self.grad_inpt)
                if norm >0.1:
                    print 'This correctly classified image can be converted into adversarial image easily'
                                
                grad_inpt_norm=self.grad_inpt/norm
                ...
                ### is curicial
                grad_inpt_norm_epislon=epislon*grad_inpt_norm
                self.a_sai=grad_inpt_norm_epislon
...
#                plt.subplot(132)
                ...
                plt.xlabel(r'$\displaystyle \epsilon=0.07$',fontsize=22, color='black')
                plt.imshow(grad_inpt_norm_epislon.reshape(28,28),plt.cm.gray)
                ...
                test_inpt= f_test_inpt(self.s) ### is a list of real values of given test input
                ####now 
                ...
                plt.xlabel(r'$\displaystyle i=543$',fontsize=22, color='black')
                plt.imshow(test_inpt.reshape(28,28), plt.cm.gray)
                
                '''x_adv= x + epislon*signum*grad(cost,x)'''
            
                self.adv_inpt=test_inpt + grad_inpt_norm_epislon
    ...
    def check_adv_inpt(self,mini_batchsize):
        ...
        if self.adv_check==0:
            ##### for checking the confidence about decision of cnn
            j=T.lscalar()
            prob_wrongly_image=theano.function(inputs=[j],outputs=self.layers[-1].confidence(),
                                     givens={
                                     self.x:
                                         self.shared_x[j*mini_batchsize:(j+1)*mini_batchsize]
                                         })
                                        
            self.prob_wrongly_classified=prob_wrongly_image(self.s)
            ####### check the gradient of cost function wrt 
            ...
            plt.imshow(self.adv_inpt.reshape(28,28), plt.cm.gray)
