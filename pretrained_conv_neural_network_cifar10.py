# -*- coding: utf-8 -*-
"""
Created on Wed Mar 02 14:51:25 2016

@author: engrs
"""

import tester_cnn_pretrained_cifa10
from tester_cnn_pretrained_cifa10 import ReLU

from tester_cnn_pretrained_cifa10 import Network
print 'wait.......'
from tester_cnn_pretrained_cifa10 import ConvPoolLayer,ConvPoolLayer1,ConvPoolLayer2,FullyConnectedLayer, SoftmaxLayer

training_data, test_data =tester_cnn_pretrained_cifa10.load_data_shared()

mini_batchsize=1
net = Network([
        ConvPoolLayer(image_shape=(mini_batchsize, 3, 32, 32), 
                      filter_shape=(32, 3, 5, 5), 
                      pool_size=(2, 2), 
                      activation_fn=ReLU),
        ConvPoolLayer1(image_shape=(mini_batchsize, 32, 14, 14), 
                      filter_shape=(32,32, 5, 5), 
                      pool_size=(2, 2), 
                      activation_fn=ReLU),
        ConvPoolLayer2(image_shape=(mini_batchsize, 32, 5, 5), 
                      filter_shape=(64, 32, 4, 4), 
                      pool_size=(2, 2), 
                      activation_fn=ReLU),
                      
        FullyConnectedLayer(n_in=64*1*1, n_out=128, activation_fn=ReLU),
#        FullyConnectedLayer(n_in=64*1*1, n_out=10, activation_fn=ReLU),

        SoftmaxLayer(n_in=128, n_out=10)], mini_batchsize)
        
net.Lets_check_Performance(mini_batchsize, test_data)
#net.SGD(training_data, 90, mini_batchsize, 0.01, test_data, lmbda=1,momentum=0.9)