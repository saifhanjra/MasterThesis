# -*- coding: utf-8 -*-
"""
Created on Mon Feb 15 12:14:10 2016
...
class Network():
    ...
    def create_adversial_inpt(self,test_data,mini_batchsize,epislon):
        if self.accurracy==1: ###if given test input is correctly classified
            ...
            if a==1:
                ...
                noise=epislon*np.random.randn(1,784)
                self.adv_inpt=test_inpt + noise
                ###### just for plotting
                plt.subplot(131)
                plt.suptitle('Adding Normally distributed Random noise to correctly classified but living on the edge inputs(close to descision boundry), Index of input=test_x[6755] ')
                plt.title('Correctly Classified input')
                plt.imshow(test_inpt.reshape(28,28), plt.cm.gray)
                plt.subplot(132)
                plt.title('Normally distributed noise, Epislon 0.6')
                plt.imshow(noise.reshape(28,28),plt.cm.gray)
                plt.subplot(133)
                plt.title('Badly distorted but still correctly classified')
    ...
    def check_adv_inpt(self,mini_batchsize):
        ...
        if self.adv_check==0:
            print 'Adversarial input has been not been created however  high distortions has made it wrongly classified'
            j=T.lscalar()
        ...
        if self.adv_check==1:
            print 'Cool, Still correctly classified. CNNs are too reliable'
            jj=T.lscalar()
            prob_distorted_correctly_classified=theano.function(inputs=[jj],outputs=self.layers[-1].confidence(),
                                     givens={
                                     self.x:
                                         self.shared_x[jj*mini_batchsize:(jj+1)*mini_batchsize]
                                         })
                                         
                                         
            self.prob_distorted_correctly_classified= prob_distorted_correctly_classified(self.s)
